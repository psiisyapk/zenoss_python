...
def dead_ports_api():
    final_list = []
    event_url = 'http://stas:9863d1c59@10.77.4.113:8080/zport/dmd/evconsole_router'
    data_api = {"action": "EventsRouter", "method": "query", "data": [{"limit": 200, "params": {"severity": [5], "eventGroup":"trap", "DeviceClass": ['	/Network/Router/AGR', '/Network/Switch/CSA', '/Network/Router/BBR/', '/Network/Router/ASBR/'], "eventState": [0,1]}}], "tid": 1}
    headers = {'Content-type': 'application/json', 'Accept': 'text/plain'}
    r = requests.post(event_url, data=json.dumps(data_api), headers=headers)
    res = r.json()
    if res['result']['events']:#res['result']['events'][0]['details']['deviceName'][0]
        for i in range(len(res['result']['events'])):
            final_list.append(res['result']['events'][i]['details']['deviceName'][0]+": "+res['result']['events'][i]['details']['ifDescr'][0].replace(', 10/100/Gig Ethernet SFP,', '').replace(', 10/100/Gig Ethernet SFP', ''))
...
def run_command(offset, name, from_id, cmd):
    ...
    if cmd == '/status':  
      ...
      if bs_arr:
        bs_arr_str = bs_arr[0]
        matching_prev = [s for s in read_f if bs_arr_str in s]
        arr_prev = re.findall(bs_arr_str, str(matching_prev))
        if arr_prev:
    ...
    elif any(cmd.upper() in i for i in read_f):
        matching_prev = [s for s in read_f if cmd.upper() in s]
        arr_prev = re.findall(cmd.upper(), str(matching_prev))
        if "AGR" in str(arr_prev):
            matching = [s for s in read_f if cmd.upper() in s]
            bs_arr = re.findall(r'M\w\w\w\w\w\W|2-77-\w\w-\w\w\w\w|2-50-\w\w-\w\w\w\w', str(matching))
            bs_arr_1 = re.findall(r'M\w\w\w\w\w|2-77-\w\w-\w\w\w\w|2-50-\w\w-\w\w\w\w', str(bs_arr))
            bs_arr_final = re.sub(', ', '\n', str(bs_arr_1))
            bs_arr_final = str(len(bs_arr_1))+" BS on this node: "+"\n"+bs_arr_final
            send_text(from_id, bs_arr_final)
            
        elif "CSA" in str(arr_prev):
            matching = [s for s in read_f if cmd.upper() in s]
            bs_arr = re.findall(r'M\w\w\w\w\w\W|2-77-\w\w-\w\w\w\w|2-50-\w\w-\w\w\w\w', str(matching))
            bs_arr_1 = re.findall(r'M\w\w\w\w\w|2-77-\w\w-\w\w\w\w|2-50-\w\w-\w\w\w\w', str(bs_arr))
            bs_arr_final = re.sub(', ', '\n', str(bs_arr_1))
            ...
            bs_find = re.search(cmd.upper(), str(arr_prev))
            result = bs_find.group(0)
            res = '\''+result+'\''
            matching = [s for s in read_f if cmd.upper() in s]
            for i in range(len(matching)):
                if res in matching[i]:
                    bs_arr = re.findall(r'M\w00\w\w\w\w-\w\w\w0\w', matching[i])
                    send_text(from_id, bs_arr)
